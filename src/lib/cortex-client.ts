...
export type AgentProviderType = "openai-compatible" | "anthropic" | "gemini";
export interface ProviderConfigDto { id:string; providerType:AgentProviderType; providerName:string; baseUrl:string; model:string; authMode:string; enabled:boolean; hasApiKey:boolean }
...
export const api = {
  ...
  saveAgentProviderConfig(agentId:string,input:{providerType:AgentProviderType;providerName:string;baseUrl:string;model:string;authMode:string;apiKey?:string;enabled?:boolean}): Promise<{config:ProviderConfigDto;status:ProvidersStatusDto["llm"]}> {
