...
export function parseTelegramProfile(raw: {
  displayName?: string;
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.every((v) => typeof v === "string")) {
      thinkingMessages = parsed.slice(0, 8).map((v) => v.trim()).filter(Boolean).slice(0, 8);
    }
  } catch {}
  try {
...
export function profileUpdateData(input: Record<string, unknown>): Prisma.TelegramBotProfileUpdateInput {
  ...
  for (const field of textFields) {
    if (typeof input[field] === "string") data[field] = String(input[field]).trim().slice(0, field === "welcomeText" || field === "helpText" ? 3000 : 512);
  }
  ...
  if (Array.isArray(input.thinkingMessages)) {
    const values = input.thinkingMessages.filter((v): v is string => typeof v === "string").map((v) => v.trim().slice(0, 180)).filter(Boolean).slice(0, 8);
    data.thinkingMessages = JSON.stringify(values.length ? values : TELEGRAM_PROFILE_DEFAULTS.thinkingMessages);
  }
  if (Array.isArray(input.commands)) {
    const commands = input.commands
      .filter((v): v is { command: string; description: string } => !!v && typeof v === "object" && typeof (v as any).command === "string" && typeof (v as any).description === "string")
      .slice(0, 12)
      .map((v) => ({ command: v.command.replace(/^\//, "").slice(0, 32), description: v.description.slice(0, 256) }));
  ...
  if (typeof input.welcomeBannerUrl === "string") {
    ...
    if (value && !/^https:\/\//i.test(value)) throw Object.assign(new Error("آدرس بنر باید با https شروع شود."), { status: 400 });
  }
  return data;
}
