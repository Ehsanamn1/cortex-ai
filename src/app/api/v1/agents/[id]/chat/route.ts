...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    if (!auth || auth.agentId !== agentId) {
      return applyCors(jsonError("API Key نامعتبر یا برای این ایجنت نیست.", 401), req.headers.get("origin"));
    }

    rateLimit(req, "agent-api-" + auth.id, 60, 60_000);
    ...
    const clientId =
      (req.headers.get("x-cortex-client-id") ??
        (typeof body.clientId === "string" ? body.clientId : "api-client"))
        .trim()
        .slice(0, 120) || "api-client";
    ...
    let conversation = !startNewChat && requestedConversationId
      ? await db.conversation.findFirst({
          where: { id: requestedConversationId, agentId, channel: "api", externalUserId: clientId },
        })
      : null;
    ...
    if (!conversation) {
      conversation = await db.conversation.create({
        data: { agentId, userId: null, title: "گفتگوی API", channel: "api", externalUserId: clientId },
      });
    }
    ...
    const history = existing.slice(-12).map((m) => ({
    ...
    const inputTokens = history.reduce((sum, item) => sum + estimateTokens(item.content), 0) + estimateTokens(message);
