...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const record = await db.agentApiKey.create({
      data: { agentId: agent.id, name, keyHash: await hashAgentApiKey(key), keyPrefix: key.slice(0, 16) },
    });
    await db.auditLog.create({
      data: { workspaceId: agent.workspaceId, userId: session.user.id, action: "agent.api_key.created", entityType: "AgentApiKey", entityId: record.id },
    });
    return applyCors(jsonOk({
      key,
      ...
      warning: "این کلید فقط یک‌بار کامل نمایش داده می‌شود.",
