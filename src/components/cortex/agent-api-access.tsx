...
export function AgentApiAccess({ agentId }: { agentId:string }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="rounded-xl border border-white/[.06] bg-black/10 p-4"><div className="flex items-center gap-2 text-sm font-medium"><KeyRound className="size-4 text-primary"/>نمونه اتصال سریع</div><p className="mt-1 text-xs leading-6 text-muted-foreground">در هر کلاینت، Header زیر را بفرست و پیام را به Endpoint اختصاصی ایجنت POST کن.</p><pre dir="ltr" className="mt-3 overflow-x-auto rounded-xl border border-white/[.06] bg-black/20 p-3 text-[11px] leading-6 text-muted-foreground">{'Authorization: Bearer YOUR_API_KEY\nContent-Type: application/json\n\n{"message":"سلام","clientId":"user-123"}'}</pre></div>
